...
const getAllCurrentlyBookedVehicles = async (req, res) => {
  try {
    const query = `
      SELECT   
        v.*, 
...
const getAllVehicles = async (req, res) => {
  try {
    const query = `
      ...
      LEFT JOIN images i ON v.vehicle_id = i.vehicle_id
      LEFT JOIN userDetails ud ON v.owner_id = ud.user_id
      LEFT JOIN bookings b ON v.vehicle_id = b.vehicle_id AND v.availability = false
      GROUP BY v.vehicle_id, ud.name, ud.email, b.start_date, b.end_date
    `;
    ...
    const vehiclesWithImages = result.rows.map(vehicle => {
      ...
      if (!vehicle.availability) {
        vehicle.booking_info = {
          start_date: vehicle.start_date,
          end_date: vehicle.end_date
        };
      }

      // Remove raw booking dates from root level
      delete vehicle.start_date;
      delete vehicle.end_date;
